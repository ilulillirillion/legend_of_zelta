...
std::vector<sf::Vector2f> GenerateBoxFromSprite(sf::Sprite pSprite) {
  ...
  // Top left
  sf::Vector2f P2((pSprite.getPosition().x + pSprite.getOrigin().x), (pSprite.getPosition().y - pSprite.getOrigin().y));
  // Bottom right
  sf::Vector2f P3((pSprite.getPosition().x - pSprite.getOrigin().x), (pSprite.getPosition().y + pSprite.getOrigin().y));
  // Bottom left
  sf::Vector2f P4((pSprite.getPosition().x + pSprite.getOrigin().x), (pSprite.getPosition().y + pSprite.getOrigin().y));
...
std::vector<sf::Vector2f> GenerateBoxFromDimensions(float px, float py, float width, float height) {
  // Top right
  sf::Vector2f P1(px, py);
  // Top left
  sf::Vector2f P2(px + width, py);
  // Bottom right
  sf::Vector2f P3(px, py + height);
  // Bottom left
  sf::Vector2f P4(px + width, py + height);
